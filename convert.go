...
func GetRemoteConvertBase64(imgUrl string, tff string, imgName string, isDelImg bool, points []*PointInfo, fontColor *RGB) (string, error) {
	fmt.Println("-----------")
	fmt.Println(points)
	// get remote imageUrl
	...
	// used the gg lib,create the canvas
	dc := gg.NewContextForImage(waterImage)
	//dc.SetRGBA(1, 1, 1, 0)
	if fontColor != nil {
		dc.SetRGB(fontColor.R, fontColor.G, fontColor.B)
	}
	...
	if len(points) > 0 {
		...
		for _, point := range points {
			...
			default:
				// if your can`t input the correct value,set the font align center
				align = gg.AlignCenter
			}
...
func Base64StrHandler2Base64(base64Str string, tff string, imgName string, isDelImg bool, points []*PointInfo, fontColor *RGB) (string, error) {
	...
	img := base64Str
	if i != -1 {
		// prefix is "data:image/png;base64,need to split this bas64 string
		img = base64Str[i+1:]
	}
	dec := base64.NewDecoder(base64.StdEncoding, strings.NewReader(img))
